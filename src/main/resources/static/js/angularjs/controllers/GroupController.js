 ...
 */
app.controller("groupController", ['NgTableParams', '$resource','$http','$scope', 'GroupService','DTOptionsBuilder',function (NgTableParams, $resource, $http, $scope,GroupService  ) {
    $scope.groups = [];
    $scope.pageno = 1;
    $scope.global={
        tableParams: null,
        ...
        getData: function (params) {
            return $scope.global.api.get(params.url()).$promise.then(function (data) {
                if (data && (data.content !== null || data.content !== undefined)){
                    ...
                    // TODO:  deal when there is no data
                    console.log(data)
