                        ...
                        <table ng-table="global.tableParams" class="table table-striped table-bordered table-striped table-condensed table-hover">
                            <tr ng-repeat="user in $data track by user.id">
                                ...
                                <td style="vertical-align: middle; text-align: center;" data-title="'Aksyon'">
                                    <a class="btn btn-warning btn-xs load" title="Aktyalize" href="/user/2/update/{{user.id}}">
                                        <i class="fa fa-edit"></i>
                                    </a>

                                    <a class="btn btn-danger btn-xs" id="delete" title="Elimine" onclick="onDelete(event)" href="/user/2/delete/{{user.id}}">
                                        <i class="fa fa-trash-o"></i>
                                    </a>
                                    <a class="btn btn-{{user.enabled? 'primary' : 'default' }} btn-xs" title="{{user.enabled? 'Bloke' : 'Debloke'}}" id="block" onclick="onBlock(event)" href="/configuration/2/user/{{user.id}}">
